...
async function renderMembers(){const q=$('memberSearch').value.toLowerCase().trim(),raw=(await all('members')).filter(m=>!q||m.name.toLowerCase().includes(q)).sort((a,b)=>a.name.localeCompare(b.name)),tx=await activeTx(),dues=await all('dues');const seen=new Set(),ms=[];for(const m of raw){const k=normName(m.name);if(seen.has(k))continue;seen.add(k);ms.push(m)}$('memberList').innerHTML=ms.map(m=>{const total=tx.filter(x=>x.type==='income'&&txBelongsToMember(x,m)).reduce((s,x)=>s+(+x.amount||0),0),d=dues.find(x=>x.memberId===m.id&&x.month===ym()),st=d?(+d.amountPaid>=+d.amountDue?'Al día':+d.amountPaid>0?'Parcial':'Pendiente'):'Sin cuota';return `<div class="item"><div><button class="member-name-link" onclick="showMemberProfile('${m.id}')">${esc(m.name)}</button><div class="item-sub">${m.status==='active'?'Activo':'Inactivo'} · ${st}</div><span class="badge">Ver ficha completa</span></div><div><div class="amount income" data-money>${money(total)}</div><button class="ghost" onclick="editMember('${m.id}')">Editar</button></div></div>`}).join('')||'<div class="empty">Sin miembros</div>'}
